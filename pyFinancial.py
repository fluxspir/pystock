...
class DbHandler:
	...
	def _addmetatable(self, metadata):
		try:
			self.exe('''insert into metatable values (?,?)
					''', metadata)
			self.commit()
			self.close()
			return
...
class StockDBHandler(DbHandler):
	...
	def _inserttable(self):
		...
		for stockinfo in self.stockinfos:
			try:
				self.exe('''insert into {} values(?,?,?)
						'''.format(self.table), stockinfo)
				self.commit()
				self.close()
...
if __name__ == "__main__":
	...
	def addprovider():
		""" """
		def getprovidertable():
			""" """
			def testprovidertable(providertable):
				""" """
				if not providertable:
					addprovider()
				if providertable.isdigit()
					i = int(providertable)
					try:
						providertable = tablelist[i-1][1]
					except IndexError:
						message = "Please use alpha numeric for table name"
						print(message)
						addprovider()
				elif providertable.isalnum():
					pass
				else:
					message = "Please use alpha numeric for table name"
					print(message)
			...
			tablelist = choosetable("provider")
			print("Which provider table you want to use ?\n\
			...
			if tablelist:
				...
				providertable = askuser("1")
			else:
				providertable = askuser("")
			
			testprovidertable(providertable)
			provider = ProviderHandler(db, providertable)
			return provider

		def getsymboltable():
			""" """
			tablelist = choosetable("symbol"


		symboltable = getsymboltable()

		provider = getprovidertable()
